% !Mode:: "TeX:UTF-8"
%% encoding: UTF-8
%% 首先读取模板的配置
\include{config/headinfo}
\include{config/william}

\usepackage{paralist}
\usepackage{longtable}
\usepackage{supertabular}

%%
\usepackage{unicode-math}  %% [bold-style=ISO]
\setmathfont{Cambria Math}

%%

\begin{document}
%% 确保zhspacing需要的符号定义没有被其他宏包改掉
\def\lq{`}\def\rq{'}
\zhspacing

%% 简写
\include{chapters/short}

%% 读入作者的信息
\include{author/author}

%% 加入pdf文件的关键字，\pdfkeywords必须在\maketitle之前才有效
\pdfkeywords{利率期限结构；人口变化；金融市场}


  % 英文封面内容
  \englishtitle{A Tale of Two Structure:\\ \Large{Implications of Demographic Changes for the Yield Curve}}
  \englishauthor{Fang Lian}
  \englishadvisor{Prof. Linlin Niu}
  \englishinstitute{Economics Department\\
	   Wangyanan Institute for Studies in Economics (WISE)\\
	   Xiamen University}
  \englishdegree{Master}
  \englishmajor{Economics}


%% 生成a4封面、原创性声明、著作权使用声明
\maketitle

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%% 术语表定义
\def\nomname{术语表}
\def\pagedeclaration#1{，第~#1~页}%
\def\eqdeclaration#1{, 方程(#1)}%
%%
%%
\makeatletter
 \newcommand{\cneqref}[1]{%
 方程（\ref{#1}）%
 }
\makeatother
%%
%%
\makeatletter
 \newcommand{\tabref}[1]{%
 表（\ref{#1}）%
 }
\makeatother
%%
%%
\makeatletter
 \newcommand{\figref}[1]{%
 图（\ref{#1}）%
 }
\makeatother
%%
%%%%% 空白页  %%%%%%%%%
\makeatletter
\def\cleardoublepage{
    \clearpage\if@twoside\ifodd\c@page\else
    \hbox{}
    \vspace*{\fill}
    \begin{center}
        %% 看什么看，没见过空白页？
        %%   
        \underline{\emph{This Draft Version:} \xxivtime \, \today} \\ %%%
        %%
        Typeset by \XeLaTeX{}. \\
        $\copyright$ Fang Lian. 
    \end{center}
    \vspace{\fill}
    \thispagestyle{empty}
    %\thispagestyle{fancy}
    \newpage
    \if@twocolumn\hbox{}\newpage\fi\fi\fi
}
\makeatother
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%% 正文默认使用小四，字号的定义在 config/headcommon.tex中
\xiaosi

%% 读入中英文摘要
\include{abstracts/cnabstract}
\include{abstracts/enabstract}
%% 生成中英文目录
\tableofcontents

  % 插图目录
  \listoffigures
  \addcontentsline{toc}{chapter}{\listfigurename} %将表格索引加入全文目录

  % 表格目录
  \listoftables
  \addcontentsline{toc}{chapter}{\listtablename}  %将图索引加入全文目录
%%% 输出术语表：Notations
%\include{chapters/symbol}
\addcontentsline{toc}{chapter}{术语表}%在目录中添加“术语表”的标记
\printnomenclature%
%% 以数字风格开始正文页码
\newpage
\pagenumbering{arabic}
\setcounter{page}{1}

%% 读入各章节
\include{chapters/chapter01}
\include{chapters/chapter02}
\include{chapters/chapter03}
\include{chapters/chapter04}
%\include{chapters/chapter05}
\include{chapters/chapter06}
%% 参考文献
  % 使用 BibTeX
   \bibliographystyle{plainnat}
   %\bibliographystyle{GBT7714-2005NLang}
    \setcitestyle{aysep={},yysep={;}}
  \bibliography{reference/tale,reference/tex,reference/demography,reference/ecmt}



%% 发表的论文
\include{appendix/publications}

%% 最后致谢一下
\include{appendix/thanks}

\end{document}
